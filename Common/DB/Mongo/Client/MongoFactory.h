...
namespace mongo
{
	...
	public:
		...
		static std::unique_ptr<Request> Insert(const std::string & tab, bson::w::Document & document);
		static std::unique_ptr<Request> Query(const std::string & tab, const json::r::Document & json, int limit = 1, int skip = 0);
		static std::unique_ptr<Request> CreateIndex(const std::string & tab, const std::string & key, int sort = 1, bool unique = false);
		static std::unique_ptr<Request> Delete(const std::string & tab, bson::w::Document & document, int limit = 1);
		static std::unique_ptr<Request> Command(const std::string & tab, const std::string & cmd, const json::r::Document & json);
		static std::unique_ptr<Request> Update(const std::string & tab, bson::w::Document & select,  bson::w::Document & update, const char * tag, bool upsert, bool multi);
	};
}
