...
namespace GameKeeper
{
    ...
    bool BindLuaFunction::Invoke(GameObject *gameObject, const std::string &jsonString)
    {
        ...
        GKAssertRetFalse_F(lua_isfunction(this->mLua, -1));
        ...
        GKAssertRetFalse_F(lua_getfunction(this->mLua, "JsonUtil", "Decode"));
        ...
        if (lua_pcall(this->mLua, 1, 1, 0) == 0)
        {
            ...
            {
                GKDebugError(lua_tostring(this->mLua, -1));
                return false;
            }
            return true;
        }
...
namespace GameKeeper
{
    bool BindClientLuaFunction::Invoke(GameObject *gameObject, const std::string &data)
    {
        ...
        GKAssertRetVal(desc, this->mPbName, false);
        Message *message = MessageFactory::generated_factory()->GetPrototype(desc)->New();
        GKAssertRetFalse_F(message->ParseFromString(data));
        std::string jsonData = "";
        GKAssertRetFalse_F(util::MessageToJsonString(*message, &jsonData).ok());
