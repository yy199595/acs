...
unpack_dict(lua_State *L, struct bson_reader *br, bool array) {
	...
	for (;;) {
		...
		case BSON_STRING: {
			...
			lua_pushlstring(L, (char *)read_bytes(L, &t, sz), sz-1);
			break;
		}
		case BSON_DOCUMENT:
		...
		case BSON_BINARY: {
			...
			luaL_addlstring(&b, (char *)read_bytes(L, &t, sz), sz);
			luaL_pushresult(&b);
			break;
		}
		case BSON_OBJECTID:
...
typeclosure(lua_State *L) {
	...
	int i;
	int n = sizeof(_typename)/sizeof(_typename[0]);
	for (i=0;i<n;i++) {
		lua_pushstring(L,_typename[i]);
	}
