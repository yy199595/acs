...
namespace values {
	...
	extern int push_null(lua_State* L);

	extern bool isnull(lua_State* L, int idx);

	extern bool hasJsonType(lua_State* L, int idx, bool& isarray);

	extern bool isarray(lua_State* L, int idx, bool empty_table_as_array = false);
