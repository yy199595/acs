...
/* Add padding and return the message digest. */
		...
		static inline void xor_key(uint8_t key[BLOCKSIZE], uint32_t xor1)
		{
			int i;
...
namespace Helper
{
	...
	std::string Sha1::GetHMacHash(const std::string& key, const std::string& text)
	{
		...
		unsigned char digest1[SHA1_DIGEST_SIZE];
		...
		if (key.size() > BLOCKSIZE) {
			SHA1_CTX ctx;
			sat_SHA1_Init(&ctx);
			sat_SHA1_Update(&ctx, (unsigned  char *)key.c_str(), key.size());
		...
		sat_SHA1_Update(&ctx2, (unsigned char *)text.c_str(), text.size());
		sat_SHA1_Final(&ctx2, digest2);

		sat_SHA1_Update(&ctx1, digest2, SHA1_DIGEST_SIZE);
		sat_SHA1_Final(&ctx1, digest1);

		return std::string((const char *)digest1, SHA1_DIGEST_SIZE);
	}
