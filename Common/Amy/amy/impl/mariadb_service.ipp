...
inline mariadb_service::mariadb_service(asio::io_service& io_service)
...
inline void mariadb_service::construct(implementation_type& impl) {
  impl.ev_ =
      std::make_unique<asio::posix::stream_descriptor>(get_io_service());
  impl.timer_ = std::make_unique<asio::steady_timer>(get_io_service());
}
...
inline std::string mariadb_service::error_message(
    implementation_type& impl, asio::error_code const& ec) {
...
inline asio::error_code mariadb_service::open(
    implementation_type& impl, asio::error_code& ec) {
...
asio::error_code mariadb_service::connect(implementation_type& impl,
...
BOOST_ASIO_INITFN_RESULT_TYPE(ConnectHandler, void(asio::error_code))
mariadb_service::async_connect(implementation_type& impl,
  ...
  if (!is_open(impl)) {
    asio::error_code ec;
    if (!!open(impl, ec)) {
      asio::post(this->get_io_service().get_executor(),
...
inline asio::error_code mariadb_service::query(
...
BOOST_ASIO_INITFN_RESULT_TYPE(QueryHandler, void(asio::error_code))
...
BOOST_ASIO_INITFN_RESULT_TYPE(
    StoreResultHandler, void(asio::error_code, amy::result_set))
...
BOOST_ASIO_INITFN_RESULT_TYPE(
    Handler, void(asio::error_code, amy::result_set))
...
inline asio::error_code mariadb_service::autocommit(
    implementation_type& impl, bool mode, asio::error_code& ec) {
  namespace ops = amy::detail::mysql_ops;
  ops::mysql_autocommit(&impl.mysql, mode, ec);
  return ec;
}

inline asio::error_code mariadb_service::commit(
    implementation_type& impl, asio::error_code& ec) {
  namespace ops = amy::detail::mysql_ops;
...
inline asio::error_code mariadb_service::rollback(
    implementation_type& impl, asio::error_code& ec) {
  namespace ops = amy::detail::mysql_ops;
...
asio::error_code mariadb_service::set_option(implementation_type& impl,
    Option const& option, asio::error_code& ec) {
  namespace ops = detail::mysql_ops;
...
void async_wait_mysql(int& status, T1& p, T2& self) {
  ...
  using asio::posix::descriptor_base;
  ...
  if (status & ops::wait_type::timeout) {
    auto& impl = p.impl_;
    if (!impl.timer_)
      impl.timer_ = std::make_unique<asio::steady_timer>(p.ioc_);
      ...
      case S_ENTRY: {
        ...
        if (p.impl_.first_result_stored) {
          ...
          mariadb_service& service =
              asio::use_service<mariadb_service>(p.ioc_);
