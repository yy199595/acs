...
namespace Tendo
{
    bool SrvRpcConfig::OnLoadJson(rapidjson::Document& document)
    {
    ...
    bool SrvRpcConfig::OnReLoadJson(rapidjson::Document& document)
    {
        return false;
    }

    const RpcServiceConfig *SrvRpcConfig::GetConfig(const std::string &name) const
    {
        auto iter = this->mConfigs.find(name);
        return iter != this->mConfigs.end() ? iter->second.get() : nullptr;
    }

    const RpcMethodConfig *SrvRpcConfig::GetMethodConfig(const std::string &fullName) const
    {
...
namespace Tendo
{
    bool HttpConfig::OnLoadJson(rapidjson::Document& document)
    {
    ...
    bool HttpConfig::OnReLoadJson(rapidjson::Document& document)
    {
        return true;
    }
