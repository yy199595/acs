...
namespace acs
{
	...
	bool OssComponent::Awake()
	{
		LuaCCModuleRegister::Add([](Lua::CCModule & ccModule) {
			ccModule.Open("util.oss", lua::lib::luaopen_loss);
		});
	...
	bool OssComponent::Delete(const std::string& objectKey)
	{
		return this->Delete(this->mConfig, objectKey);
	}

	bool OssComponent::Delete(const oss::Config& config, const std::string& objectKey)
	{
		...
		std::unique_ptr<http::Response> httpResponse = this->Run(url, std::move(httpRequest));
	...
	std::unique_ptr<oss::List> OssComponent::List(const std::string& dir, int count)
	{
		return this->List(this->mConfig, dir, count);
	}

	std::unique_ptr<http::Response> OssComponent::Run(const std::string& url, std::unique_ptr<http::Request> request)
	{
		if(!request->SetUrl(url))
		{
			return nullptr;
		}
		std::unique_ptr<http::Response> httpResponse = this->mHttp->Do(std::move(request));
	...
	std::unique_ptr<oss::List> OssComponent::List(const oss::Config& config, const std::string& dir, int count)
	{
	...
	bool OssComponent::Download(const std::string& objectKey, const std::string & path)
	{
		return this->Download(this->mConfig, objectKey, path);
	}

	bool OssComponent::Download(const oss::Config& config, const std::string& objectKey, const std::string & path)
	{
		...
		std::unique_ptr<http::Response> response = this->mHttp->Do(std::move(httpRequest), std::move(fileContent));
	...
	std::unique_ptr<http::Request> OssComponent::New(const char* method,
	...
	std::unique_ptr<oss::Response> OssComponent::Upload(const oss::Config & config, const std::string& path, const std::string & dir)
	{
		...
		do
		{
			...
			httpRequest->SetBody(std::move(fileContent));
			std::unique_ptr<http::Response> response = this->Run(url, std::move(httpRequest));
			if (response == nullptr)
			{
