...
namespace Sentry
{
	class MongoTask : public IRpcTask<Bson::BsonObject>
	{
	public:
	...
	public:
		...
		void OnResponse(std::shared_ptr<Bson::BsonObject> response) final;
		std::shared_ptr<Bson::BsonObject> Await() { return mTask.Await(); }
	private:
		int mTaskId;
		TaskSource<std::shared_ptr<Bson::BsonObject>> mTask;
	};
}

namespace Sentry
{
	class MongoComponent : public RpcTaskComponent<Bson::BsonObject>, public IStart
	{
	public:
	...
	public:
		std::shared_ptr<Bson::BsonObject> Run(std::shared_ptr<MongoRequest> request, int flag = 0);
	private:
