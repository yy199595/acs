...
namespace json
{
	namespace w
	{
		...
		public:
			...
			bool Add(const char * key, yyjson_val * v);
			bool Add(const char * key, yyjson_mut_val * v);
			bool Add(const char * key, const std::string & v);
			bool Add(const char* key, const char* str, size_t size);
			bool Add(const char* key, const std::vector<int> & value);
	...
	namespace r
	{
		...
		public:
			inline const std::string & GetError() { return this->mError; }
		private:
			yyjson_doc * mDoc;
			std::string mError;
		};
