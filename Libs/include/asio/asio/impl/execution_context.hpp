...
{
  // Check that ServiceName meets the necessary type requirements.
...
ServiceName& make_service(execution_context& e, ASIO_MOVE_ARG(Args)... args)
{
  detail::scoped_ptr<ServiceName> svc(
      new ServiceName(e, ASIO_MOVE_CAST(Args)(args)...));
  e.service_registry_->template add_service<ServiceName>(svc.get());
