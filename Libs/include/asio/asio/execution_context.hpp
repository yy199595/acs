 ...
 *
 * In a call to @c use_service<ServiceName>(), the type argument chooses a service,
 * making available all members of the named type. If @c ServiceName is not present
 * in an execution_context, an object of type @c ServiceName is created and added
 ...
 * has_service<ServiceName>().
 *
 * ServiceName objects may be explicitly added to an execution_context using the
 * function template @c add_service<ServiceName>(). If the @c ServiceName is already
...
public:
   ...
   */
  template <typename ServiceName, typename... Args>
  friend ServiceName& make_service(execution_context& e, Args&&... args);

#elif defined(ASIO_HAS_VARIADIC_TEMPLATES)

  template <typename ServiceName, typename... Args>
  friend ServiceName& make_service(execution_context& e,
